...
export default function Home() {
  ...
  const isInLastRow = (index: number, totalImages: number) => {
    const itemsPerRow = 4;
    ...
    <div className="min-h-screen bg-[#0A0A0A] relative overflow-hidden">
      ...
      <div className="flex items-center justify-center min-h-screen bg-black text-white">
        <div className="flex flex-row items-center relative">
          ...
          <h3 className="text-gray-400 text-5xl font-medium ">
  Choose
  <br />
  <span className="text-gray-400 inline-block mx-1">from</span>  
  <span className="text-white inline-block mx-1">100+</span>  
  <br />
  <span className="text-white inline-block mt-1">Categories</span>  
</h3>

  <a 
    href="#" 
    className="mt-6 text-2xl block bg-gradient-to-r from-red-400  to-purple-400 text-transparent bg-clip-text font-bold"
  >
    Explore all categories →
  </a>
</div>
      ...
      <div className="relative z-10 container mx-auto px-4 py-12">
        ...
        <div className="flex justify-center space-x-4 mb-16">
          <button 
            className={`px-8 py-2 rounded-full focus:outline-none transition-all duration-300 ${
              activeTab === 'photos' ? 'bg-white text-black' : 'bg-[#1A1A1A] text-white hover:bg-[#252525]'
            }`}
          ...
          <button 
            className={`px-8 py-2 rounded-full focus:outline-none transition-all duration-300 ${
              activeTab === 'videos' ? 'bg-white text-black' : 'bg-[#1A1A1A] text-white hover:bg-[#252525]'
            }`}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-6xl mx-auto">
          ...
          ? images.map((image, index) => (
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: index * 0.1 }}
              className="relative group w-full md:w-[250px] h-[400px] mx-auto"
            >
